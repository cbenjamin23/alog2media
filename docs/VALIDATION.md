...
- C++/CTest exercises option parsing, help/version startup, raw timeline
  ...
  mission parsing, and an end-to-end render smoke test.
...
The eight tests are option parsing, complete help startup, version startup,
timeline parsing, geometry replay, mission configuration, and end-to-end render
smoke, plus an install-tree executable smoke test. The proof-helper unit suite
completed with ten tests passing.
...
and that mission visibility plus CLI override behavior is active. It is not a
formal pixel-for-pixel comparison against an independent pMarineViewer screen
capture; creating that reference corpus is still a fidelity gate.
...
fallback plus CLI overrides. A previous MOOS-IvP suffix incompatibility is
...
EGL jobs against the official MOOS-IvP commit
...
Python helpers, and the complete product contract on both platforms. Record
the workflow URL and result here after the branch is pushed and the first run
completes; workflow presence alone is not counted as passed validation.

## Current limitations and open gates

- The committed golden protects a reviewed synthetic composition, but is not
  yet derived from an independent pMarineViewer capture.
...
- Broader direct pMarineViewer reference scenes and additional upstream
  `VIEW_*` fixture coverage remain necessary before claiming exhaustive visual
  equivalence.
